...
double squaredLengthOfVector(vector<float>& v) {
    return std::inner_product(v.begin(), v.end(), v.begin(), 0.0);
}
long long squaredLengthOfVector(vector<int>& v) {
    return std::inner_product(v.begin(), v.end(), v.begin(), 0.0);
}
        ...
        for (int dimm = 9; dimm < 60; dimm+=2) {
            for (int diff = 3; diff < 31; diff+=5)
            {
                for (int iter = 0; iter < 5; iter++) {
                    ...
                    LLL(matrix, myfile);
                    //printVec(LLLenumerate(matrix, myfile));
                    auto lllshortest = LLLenumerate(matrix, myfile);
