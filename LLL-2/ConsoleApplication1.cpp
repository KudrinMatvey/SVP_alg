...
//template<typename T>
//int determinantOfMatrix1(Matrix<T>& mat, int n)
//{
//	int num1, num2, det = 1, index, total = 1, N = mat.size(); // Initialize result   
//
...
vector<int> gauss_reduce(vector<int>& v_new, Matrix<int>& L, Matrix<int>& S, vector<double>& lengths) {
	...
	while (it != L.end())
	{
		...
		if (lengths[i] > v_new_length &&
			squaredLengthOfVector(&(v - v_new)) <= lengths[i]) {
			S.push_back(v - v_new);
			it = L.erase(it);
...
int main() {
	...
	try {
		for (int dimm = 20; dimm < 60; dimm += 2) {
			for (int diff = 20; diff <= 60; diff += 20)
			{
				for (int iter = 0; iter < 3; iter++) {
					...
					//LLL(matrix, myfile);
					//double power = (1.0 / (float)dimm);
					//double kk = determinant(matrix, dimm);
					//kk = kk > 0 ? kk : -kk;
					//double pp = pow(kk, power);
					double pp = 1;
					float bound = 1.01 * (sqrt(dimm)) * pp;
